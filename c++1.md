...
- [5. **Operadores**](#5-operadores)
  ...
  - [**Unarios**](#unarios)
  - [Operators - in decreasing precedence order -](#operators---in-decreasing-precedence-order--)
  - [Representaciones alternativas de operadores en C++](#representaciones-alternativas-de-operadores-en-c)
- [6. **Conversión de Tipos de Datos**](#6-conversión-de-tipos-de-datos)
  - [**Conversión Implícita**](#conversión-implícita)
  - [**Conversión Explícita**](#conversión-explícita)
- [7. **Variables Constantes**](#7-variables-constantes)
- [8. **Estructuras de Control**](#8-estructuras-de-control)
...
C++ fue creado por *Bjarne Stroustrup* en AT&T Bell Laboratories a principios de los años 80 como una extensión del lenguaje C. Su objetivo era añadir nuevas capacidades, como la programación orientada a objetos, mientras mantenía la velocidad y eficiencia de C. 
...
> [!WARNING] Advertencia  
> Si colocas un comentario dentro de otro utilizando `/* ... */`, el compilador generará un error. Esto ocurre porque los comentarios de múltiples líneas no se pueden anidar. Utiliza `//` para evitar problemas. 😊
...
- `#include <iostream>` permite usar funcionalidades básicas de entrada y salida, como:
  - `std::cout` para imprimir en pantalla.
  - `std::cin` para recibir datos del usuario. 

- `#include <cmath>` proporciona funciones matemáticas avanzadas, como:
  - `sqrt()` para calcular raíces cuadradas.
  - `pow()` para calcular potencias.
  - `sin()`, `cos()`, y `tan()` para operaciones trigonométricas.

- `#include <string>` permite trabajar fácilmente con cadenas de texto, como:
...
En C++, la función principal, llamada **`main()`** (del inglés 'principal'), es el punto de entrada del programa. Su declaración estándar es `int main()`, y puede devolver un valor entero (0 para éxito) al sistema operativo. También acepta argumentos de la línea de comandos: **`argc`**, que indica el número de argumentos, y **`argv`**, un array con los argumentos como cadenas de caracteres. 
...
**3. Ejecutar el programa:**
   * Haz clic en el botón de **Run** (▶️) en la parte superior derecha o utiliza el atajo **Shift+F10**.
...
En C++, el tipo `bool` se usa para representar valores lógicos, es decir, `true` (verdadero) o `false` (falso). Es un tipo integral de 1 byte que es especialmente útil en estructuras de control y en la lógica booleana.
...
* Para crear un nuevo archivo fuente en tu proyecto `ManejoDeOperadores`, tienes dos caminos rápidos:
   -  Haz clic derecho en el proyecto `ManejoDeOperadores` en la ventana del proyecto (`Alt-1`) y selecciona `New` > `C/C++ Source File`.
...
## Representaciones alternativas de operadores en C++
